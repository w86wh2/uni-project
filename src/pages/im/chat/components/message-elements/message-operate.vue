<template>
	<view class="dialog-container">
		<view class="item-box" v-if="message.flow === 'out' && message.status === 'success'"
			@click="handleMseeage('revoke')">
			<image class="item-icon" src="/static/imIcon/revoked.svg"></image>
			<view>撤回</view>
		</view>
		<view class="item-box" v-if="message.status === 'success'" @click="handleMseeage('delete')">
			<image class="item-icon" src="/static/imIcon/delete.svg" />
			<view>删除</view>
		</view>
		...
		<view class="item-box" v-if="message.flow === 'out' && message.status === 'fail'" @click="handleMseeage('resend')">
			<image class="item-icon" src="/static/imIcon/forword.svg" />
			<view>重发</view>
		</view>

<template>
  <view class="message-audio" :class="['content content-' + message.flow]" @click="handlePlay">
    <image v-if="message.flow === 'in'" class="audio-icon audio-icon-in" src="/static/imIcon/audio-play.svg"></image>
    <view>{{ data.second }}s</view>
    <image v-if="message.flow === 'out'" class="audio-icon " src="/static/imIcon/audio-play.svg"></image>
  </view>
</template>
